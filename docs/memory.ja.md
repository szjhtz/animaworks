...
**スキルの扱い**: スキルは現在のプライミング本体ではなく、`core/skills/` のカタログ、明示的 activation、router、curator、promotion で扱う。必要なスキルは `skill` ツールや active skill context 経由で本文またはポインタとして提示される。手続き記憶（`procedures/`）は引き続き長期記憶として残り、実績のある手順は promotion によって `skills/` へ昇格できる。
...
`core/memory/skill_metadata.py` には description ベースのマッチング関数が残っており、互換性や一部ユーティリティで使える。ただし、現在の自動想起はスキルを直接注入する設計ではない。必要な時は `skill` ツール、active skill context、または Skill Hub のカタログを経由して読み込む。
